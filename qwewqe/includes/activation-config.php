...
class ic_cat_activation_wizard extends ic_activation_wizard {
	...
	function activation_notices() {
		...
		if ( is_ic_activation_notice() && !$this->get_notice_status( 'notice-ic-catalog-activation' ) ) {
			...
			if ( !empty( $questions ) ) {
				$this->box_header( sprintf( __( '%s is active now!', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME ) );
			...
			if ( !empty( $listing_id ) ) {
				...
				$message		 = sprintf( __( 'Your main %s listing page is not selected. You can use shortcodes to display your products but in most cases it will be more convienient to use the templates.', 'post-type-x' ), $catalog_names[ 'singular' ] );
	...
	function woo_choice_response() {
		...
		$response[ 'question' ]	 = sprintf( __( 'It looks like you have also %s active. Make a choice below for a correct setup.', 'post-type-x' ), 'WooCommerce' ) . '<br><br>' . sprintf( __( 'I would like to use %s to:', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME );
		return $response;
	}
