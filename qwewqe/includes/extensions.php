...
function ic_epc_extensions_menu_elements() {
	?>
	<a id="extensions" class="nav-tab" href="<?php echo admin_url( 'edit.php?post_type=al_product&page=extensions.php&tab=product-extensions' ) ?>"><?php _e( 'Installation', 'post-type-x' ); ?></a>
	<?php
	/*
...
function product_extensions() {
	...
	<div id="implecode_settings" class="wrap">
		...
		<h3><?php _e( 'All premium extensions come with premium support provided by dev team.<br>Feel free to contact impleCode for configuration help, troubleshooting, installation assistance and any other plugin support at any time!', 'post-type-x' ) ?></h3>
			...
			if ( $tab == 'product-extensions' OR $tab == '' ) {
				...
				<div class="helpers">
					<div class="wrapper"><h2><?php _e( 'Did you Know?', 'post-type-x' ) ?></h2><?php
						...
						text_helper( '', __( 'Paste your license key and click install button to start the extension installation process.', 'post-type-x' ) );
						?>
					</div>
				</div> <?php
				...
				<div class="help">
					...
					echo '<p>In case you prefer to install the extension manually you will get also the installation files in the customer panel. See <a href="https://implecode.com/wordpress/product-catalog/plugin-installation-guide/?cam=extensions-help&key=manual-installation#manual">manual installation guide</a> for this.</p>';
...
function start_implecode_install() {
	if ( isset( $_GET[ 'implecode_install' ] ) && !empty( $_GET[ 'slug' ] ) && !empty( $_GET[ 'license_key' ] ) && current_user_can( 'install_plugins' ) ) {
		...
		if ( is_wp_error( $api ) && isset( $api[ 'errors' ] ) ) {
			$wp_errors = '';
			foreach ( $api[ 'errors' ] as $error_name => $wp_error ) {
				$wp_errors .= $error_name . ': ' . implode( ', ', $wp_error ) . '<br>';
			}
			...
			echo '<div id="message error" class="error product-adder-message messages-connect">
				<div class="squeezer">
					<h4><strong>' . sprintf( __( 'This is not a valid license key! Get it <a href="%s">here</a>.', 'post-type-x' ), 'https://implecode.com/wordpress/plugins/' . $_GET[ 'url' ] . '/#cam=extensions&key=' . $_GET[ 'url' ] ) . '</strong></h4>
				</div>
			</div>';
		} else {
			echo '<div id="message error" class="error product-adder-message messages-connect">
				<div class="squeezer">
					<h4><strong>' . sprintf( __( 'The supplied license key is not valid for this extension! Upgrade it <a href="%s">here</a>.', 'post-type-x' ), 'https://implecode.com/wordpress/plugins/' . $_GET[ 'url' ] . '/#cam=extensions&key=' . $_GET[ 'url' ] ) . '</strong></h4>
				</div>
			</div>';
		}
		...
		echo '<div id="message error" class="error product-adder-message messages-connect">
				<div class="squeezer">
					<h4><strong>' . sprintf( __( 'You need to provide the license key to activate the extension. Get yours <a href="%s">here</a>.', 'post-type-x' ), 'https://implecode.com/wordpress/plugins/' . $_GET[ 'url' ] . '/#cam=extensions&key=' . $_GET[ 'url' ] ) . '</strong></h4>
				</div>
			</div>';
...
function ic_cat_extension_activated_message() {
	if ( !empty( $_GET[ 'ic_cat_extension_activated' ] ) ) {
		?>
		<div id="message" class="updated notice is-dismissible"><p><?php _e( 'Extension <strong>activated</strong>.', 'post-type-x' ) ?></p></div>
		<?php
	}
}
...
if ( !function_exists( 'ic_license_renewal_button' ) ) {
	...
	function ic_license_renewal_button( $license_key = null, $button = 'button-primary', $label = null ) {
		...
		if ( empty( $label ) ) {
			$label = __( 'Renew License', IC_FRAMEWORK_TEXTDOMAIN );
		}
				...
				if ( $current_time > $in_to_weeks ) {
					...
					foreach ( $all_ic_plugins as $plugin ) {
						if ( !is_plugin_license_active( $plugin[ 'slug' ] ) ) {
							//$message = __( 'Your license is expired. Use the button below to fix it. The latest security and feature updates cannot be applied without an active license.', 'post-type-x' );
							break;
						}
					}
				}
