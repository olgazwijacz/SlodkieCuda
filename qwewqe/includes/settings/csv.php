...
function implecode_custom_csv_menu() {
	?>
	<a id="csv-settings" class="element" href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=product-settings&submenu=csv' ) ?>"><?php _e( 'Import / Export', 'post-type-x' ); ?></a>
	<?php
}
...
function simple_upload_csv_products_file() {
	...
	if ( isset( $_FILES[ 'product_csv' ] ) && ($_FILES[ 'product_csv' ][ 'size' ] > 0) ) {
		...
		echo '<form method="POST" enctype="multipart/form-data"><input type="file" accept=".csv" name="product_csv" id="product_csv" /><input type="submit" class="button" value="' . __( 'Import Now', 'post-type-x' ) . '" /></form>';
